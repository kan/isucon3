...
cat $HOME/webapp/perl/warmup_url.txt | xargs -P10 -t -n1 /usr/bin/curl > /dev/null
cat $HOME/webapp/perl/warmup_url.txt | sort -r | xargs -P10 -t -n1 /usr/bin/curl > /dev/null
